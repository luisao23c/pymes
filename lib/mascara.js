...
const SHAPE_DEFS = {
  ...
  custom:   { label: 'Personalizar',  clip: function(p){ return 'polygon('+p.map(function(v){return v[0]+'% '+v[1]+'%';}).join(',')+')'; }, pts: function(){ return [[50,2],[97,50],[50,98],[3,50]]; }, svg: 'M20,2 L38,20 L20,38 L2,20 Z' }
};

function getShapeClip(variant, points) {
  if (points && variant === 'custom') return SHAPE_DEFS.custom.clip(points);
...
.pv-mascara .mascara-preview{position:absolute;inset:0;clip-path:inherit;-webkit-clip-path:inherit;border:2px dashed #ea580c;pointer-events:none;z-index:2;}
